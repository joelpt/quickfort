...
class Grid:
    ...
    def str_commands(self, column_separator):
        rowstrings = [column_separator.join(['.' if c.command == '' else '_' if c.command is None else c.command for c in row]) + '|' for row in self.cells]
        return '\n'.join(rowstrings)

    def str_plottable(self):
        rowstrings = [''.join(['.' if c.plottable == True else 'x' if c.plottable == False else '_' for c in row])  + '|' for row in self.cells]
        return '\n'.join(rowstrings)
    ...
    def str_area_labels(self):
        rowstrings = [''.join(['.' if c.label == '' else '_' if c.label is None else c.label for c in row]) + '|' for row in self.cells]
        return '\n'.join(rowstrings)
